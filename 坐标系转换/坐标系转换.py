...
if __name__ == '__main__':
    lng = 128.543
    lat = 37.065
    df=pd.read_excel('sentiment_poi.xlsx')
    longitude_o=df['longitude']
    latitude_o=df['latitude']
    GCJ02=[]
    for i in range(len(longitude_o)):
        GCJ02.append(bd09_to_gcj02(longitude_o[i],latitude_o[i]))
        
    df['GCJ02']=GCJ02
    df.to_excel('坐标纠偏.xlsx')
